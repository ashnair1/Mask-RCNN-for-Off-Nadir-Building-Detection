...
from PIL import Image, ImageDraw, ImageFont # added by JX
...
def save_instances(image, boxes, masks, class_ids, class_names,
    ...
    masked_image = image.astype(np.uint8).copy() # <class 'numpy.ndarray'>
    #print("type(masked_image) = {}".format(type(masked_image)))
    ...
    for i in range(N):
        ...
        cv2.putText(masked_image, caption,(x1,y1+8), font, 0.7, (255,155,155),1, cv2.LINE_AA)
    ...
    '''
    fig.savefig(os.path.join('/home/csjunxu/Project/sliced_mask_JX', name))

    if auto_show:
        # plt.savefig(os.path.join('/home/csjunxu/Project/sliced_mask_JX', name))
        plt.show()
    '''
    if True:
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    fig.savefig(os.path.join('/home/ashwin/Desktop/Test/output', title))

    if auto_show:
        plt.savefig(os.path.join('/home/ashwin/Desktop/Test/output', title))
        # plt.show()
...
def display_instances_orig(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None):
